...
namespace storm {
    namespace builder {
        ...
        StaminaExplicitModelBuilder<ValueType, RewardModelType, StateType>::Options::Options() : explorationOrder(storm::settings::getModule<storm::settings::modules::BuildSettings>().getExplorationOrder()) {
            // Intentionally left empty.
        }

        template <typename ValueType, typename RewardModelType, typename StateType>
        StaminaExplicitModelBuilder<ValueType, RewardModelType, StateType>::StaminaExplicitModelBuilder(std::shared_ptr<storm::generator::NextStateGenerator<ValueType, StateType>> const& generator, Options const& options) : ExplicitModelBuilder<ValueType, RewardModelType, StateType>(generator, options) {
            // Intentionally left empty.
        }

        template <typename ValueType, typename RewardModelType, typename StateType>
        StaminaExplicitModelBuilder<ValueType, RewardModelType, StateType>::StaminaExplicitModelBuilder(storm::prism::Program const& program, storm::generator::NextStateGeneratorOptions const& generatorOptions, Options const& builderOptions) : StaminaExplicitModelBuilder(std::make_shared<storm::generator::PrismNextStateGenerator<ValueType, StateType>>(program, generatorOptions), builderOptions) {
            // Intentionally left empty.
        }

        template <typename ValueType, typename RewardModelType, typename StateType>
        StaminaExplicitModelBuilder<ValueType, RewardModelType, StateType>::StaminaExplicitModelBuilder(storm::jani::Model const& model, storm::generator::NextStateGeneratorOptions const& generatorOptions, Options const& builderOptions) : StaminaExplicitModelBuilder(std::make_shared<storm::generator::JaniNextStateGenerator<ValueType, StateType>>(model, generatorOptions), builderOptions) {
            // Intentionally left empty.
        }

        template <typename ValueType, typename RewardModelType, typename StateType>
        std::shared_ptr<storm::models::sparse::Model<ValueType, RewardModelType>> StaminaExplicitModelBuilder<ValueType, RewardModelType, StateType>::build() {
            /* This is literally copied from storm::builder::ExplicitModelBuilder, but it's the only way I can figure out how to 
            override the buildModelFromComponents() and buildMatrices() methods, which are private */
        ...
        template <typename ValueType, typename RewardModelType, typename StateType>
        StateType StaminaExplicitModelBuilder<ValueType, RewardModelType, StateType>::getOrAddStateIndex(CompressedState const& state) {
#ifdef DEBUG_PRINTS
            std::cout << "std::bind dereferenced correctly to getOrAddStateIndex. (StaminaExplicitModelBuilder)" << std::endl;
#endif
            StateType actualIndex = static_cast<StateType>(state.getAsInt(0, 64));  // get 64 bits starting at bit 0
#ifdef DEBUG_PRINTS
            std::cout << "Got actualIndex: " << actualIndex << " (in getOrAddStateIndex (StaminaExplicitModelBuilder))" << std::endl;
#endif
            return actualIndex;
        }

        template <typename ValueType, typename RewardModelType, typename StateType>
        void StaminaExplicitModelBuilder<ValueType, RewardModelType, StateType>::buildMatrices(storm::storage::SparseMatrixBuilder<ValueType>& transitionMatrixBuilder, std::vector<RewardModelBuilder<typename RewardModelType::ValueType>>& rewardModelBuilders, StateAndChoiceInformationBuilder& stateAndChoiceInformationBuilder) {
            // Copy-pasted an entire method because we need to change *one line*
            ...
            std::function<StateType (CompressedState const&)> stateToIdCallback = std::bind(&StaminaExplicitModelBuilder<ValueType, RewardModelType, StateType>::getOrAddStateIndex, this, std::placeholders::_1);
        ...
        template <typename ValueType, typename RewardModelType, typename StateType>
        storm::storage::sparse::ModelComponents<ValueType, RewardModelType> StaminaExplicitModelBuilder<ValueType, RewardModelType, StateType>::buildModelComponents() {

            // Again, had to copy-paste a method since it was private and we needex to change something.
        ...
        template <typename ValueType, typename RewardModelType, typename StateType>
        storm::models::sparse::StateLabeling StaminaExplicitModelBuilder<ValueType, RewardModelType, StateType>::buildStateLabeling() {
            return generator->label(stateStorage, stateStorage.initialStateIndices, stateStorage.deadlockStateIndices);
        }

        // Explicitly instantiate the class.
        template class StaminaExplicitModelBuilder<double, storm::models::sparse::StandardRewardModel<double>, uint32_t>;
        // template class ExplicitStateLookup<uint32_t>;

#ifdef STORM_HAVE_CARL
        template class StaminaExplicitModelBuilder<RationalNumber, storm::models::sparse::StandardRewardModel<RationalNumber>, uint32_t>;
        template class StaminaExplicitModelBuilder<RationalFunction, storm::models::sparse::StandardRewardModel<RationalFunction>, uint32_t>;
        template class StaminaExplicitModelBuilder<double, storm::models::sparse::StandardRewardModel<storm::Interval>, uint32_t>;
#endif
    }
}
